...
class Button:
    def __init__(self, surface, x, y, width, height, state='', function=0, colour=(255, 255, 255), hover_colour=(255, 255, 255), border=True, border_width=2, border_colour=(0,0,0), text='', font_name='arial', text_size=20, text_colour=(0,0,0), bold_text=False):
        self.x = x
        self.y = y
        self.pos = vec(x, y)
        self.width = width
    ...
    def update(self):
        if self.mouse_hovering(self.pos):
            self.hovered = True
        else:
            self.hovered = False
