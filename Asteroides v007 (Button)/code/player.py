...
class Player(pygame.sprite.Sprite):
    def __init__(self, pos, speed, layout, screen, images, space):
        super().__init__()
        self.space = space
        self.pos = pos
        self.screen = screen
        ...
        self.shield_max_time = 3000
        self.shield_start = 0
        self.shield_radius = 64
    ...
    def get_input(self):
        ...
        if keys[pygame.K_s]:
            if not self.shield_up:
                self.shield_sprite = Shield(self.space, (self.rect.centerx,self.rect.centery ), (0,0),self.shield_radius, self.layout)
                self.shield.add(self.shield_sprite)
                self.shield_up = True
                self.shield_start = pygame.time.get_ticks()

        if self.shield_up and pygame.time.get_ticks() - self.shield_start >= self.shield_max_time:
            self.shield_sprite.kill()
            self.shield_up = False

    def get_shield_up_time(self):
        if self.shield_up:
            return self.shield_max_time - (pygame.time.get_ticks() - self.shield_start)
        else:
